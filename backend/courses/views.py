...
class SubjectViewSet(viewsets.ModelViewSet):
    ...
    def enrollments(self, request, pk=None):
        ...
        if request.method == "GET":
            ...
            serializer = EnrollmentSerializer(
                data=request.data, context={"request": request, "subject": subject}
            )
    ...
    def upload_enrollments_csv(self, request, pk=None):
        ...
        try:
            result = process_enrollments_csv(
                subject, file_serializer.validated_data["file"]
            )
    ...
    def export_csv(self, request, pk=None):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{subject.code}_consolidado.csv"'
        )
...
class ExerciseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if subject_id:
            return qs.filter(subject_id=subject_id)
    ...
    def submit_solution(self, request, pk=None):
        ...
        result, created = StudentExerciseResult.objects.get_or_create(
            enrollment=enrollment, exercise=exercise, defaults={"status": "SUBMITTED"}
        )
        ...
        if not submission_file and not submission_text:
            return Response(
                {"detail": "Debes proporcionar un archivo o texto de solución."},
        ...
        if submission_file:
            ...
            if submission_file.size > 1024 * 1024:
                return Response(
                    {"detail": "El archivo excede el tamaño máximo de 1MB."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        if submission_text:
            if len(submission_text) > 5000:
                return Response(
                    {"detail": "El texto excede el límite de 5000 caracteres."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def list_results(self, request, pk=None):
        ...
        allowed = (
            ...
            or (
                ...
                and enrollment.subject.teacher_id == user.id
            )
            or enrollment.student_id == user.id
        )
        if not allowed:
            return Response({"detail": "No autorizado."}, status=403)

        results = StudentExerciseResult.objects.filter(
...
class StudentExerciseResultViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if user_role not in ["ADMIN", "TEACHER"]:
            return Response(
                {
                    "detail": "Solo profesores y administradores pueden crear resultados."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if not enrollment_id or not exercise_id or not result_status:
            return Response(
                {"detail": "enrollment, exercise y status son requeridos."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"detail": "Ejercicio no encontrado."}, status=status.HTTP_404_NOT_FOUND
            )

        # Verify the exercise belongs to the same subject as enrollment
        if exercise.subject_id != enrollment.subject_id:
            return Response(
                {"detail": "El ejercicio no pertenece a la materia del estudiante."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if user_role == "TEACHER":
            if enrollment.subject.teacher_id != user.id:
                return Response(
                    {
                        "detail": "No tienes permiso para crear resultados en esta materia."
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )

        # Check if result already exists
        existing = StudentExerciseResult.objects.filter(
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user_role not in ["ADMIN", "TEACHER"]:
            return Response(
                {
                    "detail": "Solo profesores y administradores pueden editar resultados."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if user_role == "TEACHER":
            if instance.enrollment.subject.teacher_id != user.id:
                return Response(
                    {
                        "detail": "No tienes permiso para modificar resultados de esta materia."
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
    ...
    def generate_ai_feedback(self, request):
        ...
        if not exercise_id or not result_status:
            return Response(
                {"detail": "Faltan datos requeridos (exercise_id, status)."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if getattr(user, "role", None) == "TEACHER":
            if exercise.subject.teacher_id != user.id:
                return Response(
                    {"detail": "No tienes permiso para esta materia."},
                    status=status.HTTP_403_FORBIDDEN,
                )
        ...
        try:
            ...
            enrollment = Enrollment.objects.get(
                subject=exercise.subject, student=student_user
            )
            # Find the result
            result = StudentExerciseResult.objects.get(
                enrollment=enrollment, exercise=exercise
            )
            if result.submission_file:
            ...
            StudentExerciseResult.DoesNotExist,
        ):
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def delete_all(self, request):
        ...
        return Response(
            {"message": f"{count} notificaciones eliminadas", "deleted": count}
        )
...
class StudentDashboardView(views.APIView):
    ...
    def get(self, request):
        ...
        success_rate = (
            round((green_count / total_results * 100), 1) if total_results > 0 else 0
        )
        ...
        for enrollment in enrollments:
            ...
            completion_rate = (
                round((completed_exercises / total_exercises * 100), 1)
            ...
            subjects_progress.append(
                {
                    ...
                    "yellow_count": yellow,
                    "red_count": red,
                    "success_rate": round((green / completed_exercises * 100), 1)
        ...
        all_subject_exercises = Exercise.objects.filter(
            subject_id__in=all_enrolled_subjects
        ).select_related("subject")
        completed_exercise_ids = all_results.values_list("exercise_id", flat=True)
        pending_exercises_queryset = all_subject_exercises.exclude(
            id__in=completed_exercise_ids
        ).order_by("-id")[:10]
            ...
            pending_exercises.append(
                {
                    ...
                    "deadline": exercise.deadline.strftime("%d/%m/%Y %H:%M")
        ...
        # Últimos resultados
        recent_results = all_results.select_related(
            "exercise__subject", "enrollment"
...
class CalendarViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.role == "TEACHER" and subject.teacher != self.request.user:
            raise permissions.PermissionDenied(
                "No puedes crear eventos para materias que no enseñas."
            )
        serializer.save()
    ...
    def all_events(self, request):
        ...
        exercises_qs = Exercise.objects.filter(deadline__isnull=False).select_related(
            "subject"
        )
        if getattr(user, "role", None) == "TEACHER":
